          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {selectedMode === "image" ? (
              <div className="md:col-span-2 bg-white rounded-lg shadow-md p-6">
                <div className="flex flex-col space-y-4">
                  ...
                  <button
                    ...
                    className="bg-[#007AFF] text-white px-6 py-3 rounded-lg disabled:opacity-50"
                  >
                    {loading ? (
                  ...
                  {generatedImage && (
                    <div className="mt-4">
                      ...
                      <button
                        onClick={() =>
                          handleSaveItem(
                            { url: generatedImage, prompt: searchInput },
                            "image"
                          )
                        }
                        className="mt-2 text-[#666] hover:text-[#333]"
                      >
              ...
              <div className="bg-white rounded-lg shadow-md p-6 md:col-span-2">
                <div className="flex flex-col space-y-4">
                  <div className="messages-container max-h-[400px] overflow-y-auto">
                    {messages.map((msg, index) => (
                      ...
                      >
                        {msg.content}
                        <button
                          onClick={() => handleSaveItem(msg, "chat")}
                    ...
                    {streamingMessage && (
                      <div className="bg-[#e3f2fd] p-3 rounded-lg mb-2 mr-8">
                        {streamingMessage}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      disabled={loading || !searchInput.trim()}
                      className="bg-[#007AFF] text-white px-4 py-2 rounded-lg disabled:opacity-50"
                    >
                      {loading ? (
                        <i className="fas fa-spinner fa-spin"></i>
                      ) : (
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="space-y-4">
                {recentActivity.map((activity) => (
                  <div
                    key={activity.id}
                    className="flex items-center justify-between p-3 hover:bg-[#f5f5f5] rounded"
                  >
                    <div className="flex items-center space-x-3">
                      <i
                        className={`fas fa-${
                          activity.type === "chat" ? "comment" : activity.type
                        } text-[#666]`}
                      ></i>
                      <div>
      ...
      {cameraActive && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded-lg">
            <video autoPlay className="w-full max-w-md rounded"></video>
            <button
              onClick={() => setCameraActive(false)}
              className="mt-4 bg-[#007AFF] text-white px-4 py-2 rounded-lg"
            >
              Close Camera
            </button>
          </div>
